...
;;-----TODO---------------
;;  if    <test1> then <utfall1>
;;  elsif <test2> then <utfall2>
;;  else <utfall3>)
(define (eval-if exp env)  
  (if (else? exp)
      (mc-eval (if-alternative exp) env)      
...
(define (let->lambda exp)
  ...
  (let ((bindings (cadr exp))
        (body (caddr exp)))
    (display (make-lamda bindings body))
